...
- 각 구간의 도착시간을 계산하는 기능
- 구간 추가 삭제시 순서대로 정렬하는 기능

## 요구사항

- 지하철 노선에 첫차시간, 막차시간, 출발간격을 추가한다.
    - 첫차 막차시간은 시분초까지 표기한다
        - 23:00:00
    - 출발간격은 초로 표기한다.
        - 60
    - 컨트롤러에서 첫차,막차,출발간격의 문자열을 받아 jsonConverter를 이용해 LocalTime으로 컨버팅해 사용한다
- 지하철이 특정역에 도착하는 시간조회
    - 전역에서 이동시간 + 간격 ex) 05:00 + 00::03 = 05:03
    - A역출발시간 + 걸리는시간 = 도착시간
- 도착시간 기준 최단시간 탐색
    - 가중치를 도착시간으로 변경(기존 이동시간)
    - A역 - B역 구간에서 B역 도착시간을 알려면 A역 출발시간을 알아야한다.
        1. A역이 노선의 첫역이면 노선의 출발시간,첫차시간,간격을 이용해 도착시간을 구한다
        2. A역이 첫역이 아니면 노선의 출발시간 + 총 이동시간을 이용해 구한다
            - 예시조건
                - 노선 : A -(3분) B -(4분) - C
                - 첫차시간 : 5:00
                - 출발시간: 10:00
                - 간격 : 10분
            - 계산
                - B역에 도착하는시간 : A역 출발시간ㅔ 10:00 + 이동시간 3분 = 10:03
                - C역에 도착하는시간 : A역 출발시간10:03 + 4뷴 = 10:07
    - 이전역의 도착시간 + 이동시간 = 현재역 도착시간
    - 특정역의 도착시간은 노선의정보 + 구간의 정보가 필요함
        - 노선에서 시간관련정보를 객체로 추출한다
    - Sections에 노선시간 정보를 전달해 노선의 도착시간을 구한다.
        - 경로탐색시에만 필요하므로 Sections에 메시지를 전달해 구간의 도착시간을 구하게한다 (고민필요)
